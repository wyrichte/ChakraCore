...
constexpr UINT32 crc32(const char* in) {
    UINT32 crc = (UINT32)-1;
    while(*in != '\0') {
...
void ScriptContextTelemetry::OutputTraceLogging(GUID activityId, DWORD hostType, bool isJSRT)
{
    ...
    // Handle any exceptions; see note in catch block.
    try
    {
        ...
        //
        // In addition, we have another set of telemetry data from the engine about bailout
        // and rejit numbers, and how many times we hit the cap and had to abandon a rejit,
        // or just bailed out too much. This data is _also_ stored per-ScriptContext, which
        // means that we want to send it when the ScriptContext closes (much like the other
        // one). However, the Telemetry guidelines (and the nagmail) say that Telemetry and
        // associated messages should only happen once per user event, which in these cases
        // is the closing of the page. As such, we need to group all of this together.

        // Common, useful defines
        ThreadContext* threadContext = ThreadContext::GetContextForCurrentThread();
        ScriptContext& scriptContext = this->scriptContext;
        size_t idx;
#define FIELDNAMESTRING(type, base, name) STRINGIZE(type ## _ ## base ## _ ## name)
#define TOKENJOIN2(x, y) x ## y
#define TOKENJOIN(x, y) TOKENJOIN2(x, y)
#define UNIQNAME(prefix) TOKENJOIN(prefix, __LINE__)

        // Do whatever work needs to be done to set up things to be consumed by ETW.
#define SCT_STATE 0
#include "ScriptContextTelemetryModules.h"
#undef SCT_STATE
        TELPROLOGUE("ESBuiltins")
        // Define the sizes of the name strings and the field count.
#define SCT_STATE 1
#include "ScriptContextTelemetryModules.h"
#undef SCT_STATE
        TELMIDLOGUEPART1("ESBuiltins")
        // Add the name strings and the types for the fields.
#define SCT_STATE 2
#include "ScriptContextTelemetryModules.h"
#undef SCT_STATE
        TELMIDLOGUEPART2()
        // Add up the space for all the fields.
#define SCT_STATE 3
#include "ScriptContextTelemetryModules.h"
#undef SCT_STATE
        TELMIDLOGUEPART3()

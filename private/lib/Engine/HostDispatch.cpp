...
HRESULT HostDispatch::CallInvokeEx(DISPID id, WORD wFlags, DISPPARAMS * pdp, VARIANT * pvarRes, EXCEPINFO * pei)
{
    ...
    BEGIN_LEAVE_SCRIPT_SAVE_FPU_CONTROL(scriptContext)
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            {
                IJavascriptDispatchRemoteProxy * remoteProxy = NULL;
    ...
    while(current)
    {
        if (wcscmp(current->Name, name) == 0)
        {
            ...
            BEGIN_LEAVE_SCRIPT(scriptContext)
            {
                ...
                if (punkCaller != NULL)
                {
                    IJavascriptDispatchLocalProxy* pJavascriptDispatchProxy = NULL;
...
void HostDispatch::GetReferenceByDispId(DISPID id, Js::Var *pValue, const wchar_t *name)
{
    ...
    if (FAILED(hr)) HandleDispatchError(hr, null);
...
#endif
        hr = this->CallInvokeEx(id, wFlags, &dp, NULL, &ei);
    }
    else
    {
#ifdef TEST_LOG
...
{
    // We don't need to support old style getter/setter from IDispatch code path anymore.
    Assert(false);
    return FALSE;
}
...
{
    // We really shouldn't be called with HostDispatch in edge; we don't want to support old getter/setter 
    // call in edge for hostdispatch.
    Assert(FALSE);
...
BOOL HostDispatch::DeletePropertyByDispId(DISPID id)
{
    ...
    BEGIN_LEAVE_SCRIPT(scriptContext)
    {
        ...
        if (hr == DISP_E_UNKNOWNNAME)
        {
            ...
            IJavascriptDispatchLocalProxy *pProxy = null;
        ...
        if(!fIsPut)
        {
            ...
            else
            {
                BEGIN_LEAVE_SCRIPT(scriptContext)
                {
                    ...
                    {
                        IJavascriptDispatchRemoteProxy *pRemoteProxy = null;
...
void HostDispatch::HandleDispatchError(Js::ScriptContext * scriptContext, HRESULT hr, EXCEPINFO* exceptInfo)
{
    ...
    if (DISP_E_EXCEPTION == hr)
    {
        ...
        Assert(exceptInfo != null);
        if (NULL != exceptInfo->pfnDeferredFillIn)
        {
            exceptInfo->pfnDeferredFillIn(exceptInfo);
            exceptInfo->pfnDeferredFillIn = NULL;
        }
        if (exceptInfo->bstrDescription != NULL)
        {
...
BOOL HostDispatch::GetRemoteTypeId(Js::TypeId* typeId)
{
    ...
    IJavascriptDispatchRemoteProxy* jscriptInfo = NULL;
...
{
    IJavascriptDispatchLocalProxy* jsProxy = NULL;
    HostVariant* pHostVariant = NULL;
    HRESULT hr;
    if (FAILED(hr = GetHostVariantWrapper(&pHostVariant))) return NULL;

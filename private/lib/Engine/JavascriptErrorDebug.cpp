...
namespace Js
{
    ...
    void __declspec(noreturn) JavascriptErrorDebug::SetMessageAndThrowError(ScriptContext* scriptContext, JavascriptError *pError, int32 hCode, EXCEPINFO* pei)
    {
    ...
    void __declspec(noreturn) JavascriptErrorDebug::MapAndThrowErrorWithInfo(ScriptContext* scriptContext, HRESULT hr)
    {
        ...
        {
            pError = scriptContext->GetHostScriptContext()->CreateWinRTError(perrinfo, &roerrstr);
        }
        else
#endif
        if (perrinfo != nullptr)
        {
            ...
            JavascriptErrorDebug *pErrorDebug = RecyclerNewFinalized(library->GetRecycler(), JavascriptErrorDebug, perrinfo, library->GetErrorType(errorType));
            JavascriptError::SetErrorType(pErrorDebug, errorType);
            pErrorDebug->SetRestrictedErrorStrings(&roerrstr);
            pError = pErrorDebug;
        }
        else
        {
            pError = MapError(scriptContext, errorType);
        }
        ...
        SetMessageAndThrowError(scriptContext, pError, hr, &excepinfo);
    }
    ...
    JavascriptError* JavascriptErrorDebug::CreateNewErrorOfSameType(JavascriptLibrary* targetJavascriptLibrary)
    {
        if (GetErrorType() == kjstWinRTError)
        {
            return targetJavascriptLibrary->GetScriptContext()->GetHostScriptContext()->CreateWinRTError(nullptr, nullptr);
        }
        else
        {
            return __super::CreateNewErrorOfSameType(targetJavascriptLibrary);
        }
    }

...
namespace JsStaticAPI
{
    ...
    HRESULT BGParse::ExecuteBackgroundParse(DWORD dwBgParseCookie, IActiveScriptDirect* activeScriptDirect, DWORD_PTR dwSourceContext, DWORD dwFlags, VARIANT* pvarResult, EXCEPINFO* pexcepinfo)
    {
        ScriptEngineBase* scriptEngineBase = ScriptEngineBase::FromIActiveScriptDirect(activeScriptDirect);
        return scriptEngineBase->ExecuteBackgroundParse(dwBgParseCookie, dwSourceContext, dwFlags, pvarResult, pexcepinfo);
    }
}
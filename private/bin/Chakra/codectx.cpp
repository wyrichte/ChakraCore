...
HRESULT CDebugStackFrame::Init(ScriptSite* scriptSite, int frameIndex)
{
    ...
    {
        m_framePointers = scriptContext->diagProbesContainer.GetFramePointers();
...
HRESULT CDebugStackFrame::GetCodeContext(IDebugCodeContext **ppCodeContext)
{
    return DebugApiWrapper([=] {
        ...
        if (pFunction)
        {
            ...
            if (m_frameIndex == 0 && m_scriptSite->GetScriptSiteContext()->diagProbesContainer.IsSetNextStatementCalled())
            {
                iCurrentByteCodeOffset = m_scriptSite->GetScriptSiteContext()->diagProbesContainer.GetByteCodeOffset();
            }
    ...
    )
{
    ...
    if (direction > 0)
    {
        ...
        if (!_pFuncBody->GetScriptContext()->diagProbesContainer.GetNextUserStatementOffsetForSetNext(
                                                ...
                                                IDebugCodeContext *codeContext)
{
    return DebugApiWrapper([=] {
        ...
        if (hr == S_OK)
        {
            ...
            scriptContext->diagProbesContainer.SetNextStatementAt(bytecodeAtNext);
...
CEnumDebugStackFrames::Init()
{
    ...
    BEGIN_TRANSLATE_OOM_TO_HRESULT
    {
        if (!m_scriptSite->IsClosed())
        {
            m_framePointers = m_scriptSite->GetScriptSiteContext()->diagProbesContainer.GetFramePointers();
        }
    }
    END_TRANSLATE_OOM_TO_HRESULT(hr);
    return hr;

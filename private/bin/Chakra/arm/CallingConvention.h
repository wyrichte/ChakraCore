...
// ARM Procedure Call Standard -- layout for parameters.
...
private:
    ...
    {
        int AvailableFloatRegsMask; // Bitmask of available floating point argument registers (s0-s15/d0-d7).
        ...
        RegisterAndStackState(int reservedGeneralRegisterCount) : 
            AvailableFloatRegsMask(0), GenRegIndex(reservedGeneralRegisterCount), StackSlotIndex(0)
        {
        }
    };
...
public:
    ...
    void GetNextParameterLocation(int byteCount, bool isFloatingPoint, bool is64BitAlignRequired, ParameterLocation *loc);
    ...
    // Bitmask of used float registers.
    int GetFloatRegisters()
    {
        return m_usageData.AvailableFloatRegsMask;
    }

    static bool IsFloatingPoint(RtCONCRETETYPE type);
    static bool Is64BitAlignRequired(RtCONCRETETYPE type);
};

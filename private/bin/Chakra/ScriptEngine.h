...
public:
    AutoSetDispatchHaltFlag(Js::ScriptContext *scriptContext, ThreadContext *threadContext) : m_scriptContext(scriptContext), m_threadContext(threadContext)
    {
        ...
        Assert(!m_scriptContext->diagProbesContainer.IsPrimaryBrokenToDebuggerContext());
        m_scriptContext->diagProbesContainer.SetIsPrimaryBrokenToDebuggerContext(true);
    }
    ~AutoSetDispatchHaltFlag()
    {
        ...
        Assert(m_scriptContext->diagProbesContainer.IsPrimaryBrokenToDebuggerContext());
        m_scriptContext->diagProbesContainer.SetIsPrimaryBrokenToDebuggerContext(false);
    }
private:
...
private:
    ...
    HRESULT RundownSourcesAndReparse(ULONG pairCount, SourceContextPair* pSourceContextPairs, bool shouldPerformSourceRundown, bool shouldReparseFunctions);
...
public:
    ...
    static HRESULT DbgRegisterFunction(Js::ScriptContext * scriptContext, Js::FunctionBody * functionBody, LPCWSTR title);
    static HRESULT DbgRegisterFunction(Js::ScriptContext * scriptContext, Js::FunctionBody * functionBody, DWORD_PTR dwDebugSourceContext, LPCWSTR title);
...
private:
    ...
    // This will be set to true if the dynamic attach/detach failed. Currently the failure captured only for running script.
    BOOL m_hasFailedToAttachDetach   : 1;

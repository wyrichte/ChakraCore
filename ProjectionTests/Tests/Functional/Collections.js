...
(function () {
    ...
    runner.addTest({
        id: 12,
        desc: 'VectorAsArray_Reduce_ReduceRight',
        pri: '0',
        test: function () {
            GetVector();
            myArray = [0, 22, 14, 98, 54, 55, 11, 22, 33, 44, 55, 66, 100, 100, 100, 100];
    ...
    runner.addTest({
        id: 17,
        desc: 'VectorAsArray_LastIndexOf',
        pri: '0',
        test: function () {
            GetVector();
            myArray = [98, 66, 55, 55, 54, 44, 33, 22, 22, 14, 11, 100, 100, 100, 100, 0];
    ...
    runner.addTest({
        id: 21,
        desc: 'Usage of Vector that throws error when trying to set item',
        pri: '0',
        test: function () {
            var myArray = [1, 2, 3, 4];
            var myVector = (new Animals.Animal(2)).getReadOnlyVector(myArray);
            dumpVector(myVector, "myVector");

            verify.exception(function () {
                myVector.setAt(2, 22);
            }, WinRTError, "Exception when using setAt method");

            dumpVector(myVector, "myVector");
            verify.exception(function () {
                myVector[2] = 22;
            }, WinRTError, "Exception when using index operation to set new value");
        }
    });

    Loader42_FileName = 'Collections test';
})();

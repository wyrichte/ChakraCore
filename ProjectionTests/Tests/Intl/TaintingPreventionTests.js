...
(function () {
    ...
    Array.prototype.tryGlobalTainting = function (global) {
        for (var i = 0; i < this.length; i++) {
            try{
                verify(verifyGlobalTainting(global, this[i]), this[i](), "Tainting Global '" + global + "'.");
            }
            catch (e) {
                fail("Error thrown when testing Global '" + global + "'. Message: " + e + " \nStack:\n" + e.stack);
            }
        }
    };
...
function verifyPropertyTainting(property, func, attributes) {
    ...
    Array.prototype.tryUsingGettersSetters = function (property) {
        for (var i = 0; i < this.length; i++) {
            try {
                verify(verifyPropertyTainting(property, this[i], { get: throwFunction }), this[i](), "Tainting property tainting '" + property + "' with getter.");
                verify(verifyPropertyTainting(property, this[i], { set: throwFunction }), this[i](), "Tainting property tainting '" + property + "' with setter.");
                verify(verifyPropertyTainting(property, this[i], { value: throwFunction }), this[i](), "Tainting property tainting '" + property + "' with value.");
            }
            catch (e) {
                fail("Error thrown when testing Global '" + property + "'. Message: " + e.message +"\n" + this[i] + "\nStack:" + e.stack);
            }
        }
    };
    ...
    runner.addTest({
        id: 1,
        desc: 'Tainting Constructors',
        pri: '0',
        test: function () {
            try {
                //Sanity check of the above function used for tainting
                verify.exception(function () { verifyGlobalTainting("RegExp", function () { new RegExp(); }) }, Error, "Sanity check to test helper function.");
    ...
    //Test property tainting
    runner.addTest({
        id: 2,
        desc: 'Tainting Properties',
        pri: '0',
        test: function () {
            //Sanity check of the above function used for tainting
            verify(verifyPropertyTainting("test", function () { return {}.test; }, { value: "Pass" }), "Pass", "Sanity check to test helper function.");
    ...
    Loader42_FileName = "General Intl Tests"
})();

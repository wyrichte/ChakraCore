...
Utils.getHOSTType= function(){
    ...
    //    return this.WWAHOST;
    // after IE11, navigator.appName will be 'NetScape'
    if (typeof window.location !== 'undefined' && typeof window.MSApp !== 'undefined')
        return this.WWAHOST;
    return this.IEHOST;
}
Utils.isWWAHost = function () {
    return Utils.getHOSTType() === this.WWAHOST;
}
...
///////////////////////////////////////////////////////////////////////////////
// WexLogger:
// Logs to WexLogger via WinRT
///////////////////////////////////////////////////////////////////////////////
function WexLogger() {
    var wex = Windows.WEX.Logging;
    var loggerObj = null;
    var maxMsgSize = 3000; //actually 3901, but do we really even need 3000 in wtt
    var event;
    this.setup = function (str) {
        var logSettings = new wex.LogSettings();
        logSettings.connectionData = str;
        loggerObj = new wex.Log(logSettings);
    }

    this.start = function (filename, priority) {
        //event = new Windows.Test.WWA.Feature.DefaultCapabilities.SyncEvent("wwahost");
        loggerObj.comment("Running " + filename);
    }

    this.testStart = function (test) {
        loggerObj.startGroup(test.desc);
        loggerObj.property("Priority", test.pri);
        loggerObj.property("Id", test.id);
    }

    this.verify = function (test, passed, act, exp, msg) {
        //todo: what's the right call for failure
        if (passed)
            loggerObj.comment("Passed: " + msg, test.desc)
        else
            loggerObj.assert("Failed - Act: " + act + ", Exp: " + exp + ":" + msg, test.desc);
    }

    this.pass = function (test) {
        loggerObj.result(wex.TestResult.testResult_Passed);
    }

    this.fail = function (test) {
        loggerObj.result(wex.TestResult.testResult_Failed);
    }

    this.error = function (test, error) {
        //todo: signature can be msg, context, file, function, line
        loggerObj.error("Error: " + error.name + " - " + error.description, test.desc);
        loggerObj.miniDump();
    }

    this.testEnd = function (test) {
        loggerObj.endGroup(test.desc);
    }

    this.end = function (filename, priority) {

        loggerObj.comment("End " + filename);
        //event.set();
    }
    ...
    this.comment = function (str) {
        ...
        str = str || "null";
        var msg = str.toString().substring(0, maxMsgSize);
        loggerObj.comment(msg);
    }

    this.bug = function (number, msg, db) {
        db = db || "Windows 8 Bugs";
        loggerObj.bug(db, number);
        loggerObj.comment(msg);
        loggerObj.result(wex.TestResult.testResult_Blocked);
    }
}

WexLogger.setup = function (str) {
    var logger = new WexLogger();
    logger.setup(str);
    runner.subscribe(logger);
}
//WexLogger is not included in the shouldEnable protocol due to the requirements for seting up the remote logging
...
function HTMLLogger() {
    ...
    function replaceAngleBrackets (text){
        if(typeof text == "undefined")
            return "undefined";
        text = text || "null";
        if (typeof text.toString === 'undefined')
            return "(" + typeof text + ")"; // can't get the string format of the object, logging the object type
        text = text.toString();
    ...
    this.error = function(test, error) {
        failures++;
        failureList.push(test.id);
        // based on host type, decide whether throw or not
        if (!Utils.isWWAHost())
     ...
     var Loader42Log = {
        ...
        InitLogger : function() {
            if (Loader42Log.Logger === null) {
                try {
                    Loader42Log.Logger = new ActiveXObject("Loader42.Logger");
        ...
        AllTestsComplete : function() {
            ...
            if(Loader42Log.TestsComplete) {
                ...
                Loader42Log.TestFinish("LogException", Loader42Log.FAIL, "AllTestsComplete() called twice.");
            } 
            else {
...
Loader42Logger.shouldEnable = function() {
    ...
    return false;
    try{
        var Logger = new ActiveXObject("Loader42.Logger");
        return true;
    }
    catch(e){
        return false;
...
var CrossContextTest = function () {
    var cct = this;
    cct.testIframe = true;
    cct.testWindow = (Utils.isWWAHost()) ? false : true;
    ...
    function WriteChildHtml(doc) {
        ...
        function waitForReady() {
            function handleError(e) {
                try { parentWindow.postMessage(e, "error in child context"); } catch (e) { }
            }
            try {
                document.addEventListener("DOMContentLoaded", function () {
                    try { parentWindow.childReady.ready(); }
    ...
    var writeChildContent = (Utils.isWWAHost()) ? AppendChildHtml : WriteChildHtml;
    writeChildContent = WriteChildHtml;
    ...
    // wwahost don't even dynamically adding iframe now
...
var env = "";
try {
    env = (new ActiveXObject("WScript.Shell")).Environment("Process")("JScript_Mutators");

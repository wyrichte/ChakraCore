...
(function () {
    ...
    runner.addTest({
        ...
        test: function () {
            ...
            // Verify cannot set a property after it's been deleted (extensible)
    ...
    runner.addTest({
        ...
        test: function () {
            ...
            // Verify cannot delete properties from prototype
            deleteAndCheckProperty(animalPrototype, "id", false);
        }
    });
    ...
    runner.addTest({
        ...
        test: function() {
            ...
            try {
                ...
                verify(false, true, "addEventListener"); // fail
            }
            catch (e) {
                verify(false, false, "addEventListener"); // expected
            }

            try {
                Object.defineProperty(Windows.UI.WebUI.WebUIApplication, "onactivated", {
                                      value: function(handler) { numberOfEventListeners++;}});
                verify(false, true, "onactivated"); // fail
            }
            catch (e) {
                verify(false, false, "onactivated"); // expected
            }

            Windows.UI.WebUI.WebUIApplication.addEventListener("activated", null);
            verify(numberOfEventListeners, 0, "Failed to overwrite a WinRT addEventListener");

            try {
                Windows.UI.WebUI.WebUIApplication.onactivated(null);
                verify(false, true, "onactivated(null)"); // fail
            }
            catch (e) {
                verify(false, false, "onactivated(null)"); // expected
            }
            ...
            try {
                ...
                verify(false, true, "createAttribute"); // fail
            }
            catch (e) {
                verify(false, false, "createAttribute"); // expected
            }
        }
    });
    ...
    runner.addTest({
        ...
        test: function() {
            ...
            try {
                ...
                verify(false, true, "getNumFins"); // fail
            }
            catch (e) {
                verify(false, false, "getNumFins"); // expected
            }

            verify(myFish.getNumFins(), 102, "myFish.getNumFins()");
            Animals.Animal.myFish = myFish;
            verify(Animals.Animal.callMyFishMethod(102), true, "Animals.Animal.callMyFishMethod(102)");
            verify(Animals.Animal.myFish.getNumFins(), 102, "Animals.Animal.myFish.getNumFins()");
        }
    });
    ...
    runner.addTest({
        ...
        test: function() {
            ...
            // Verify no modifications persist when original object is not collected
            ...
            var myFish = new Animals.Fish();
            verify.notEqual(myFish.getNumFins, 23, "myFish.getNumFins");
            Animals.Animal.myFish = myFish;
            var returnedFish = Animals.Animal.myFish;
            verify.strictEqual(returnedFish, myFish, "Animals.Animal.myFish === myFish");
            verify.typeOf(returnedFish.getNumFins, 'function');
            verify.notEqual(returnedFish.getNumFins, 23, "Animals.Animal.myFish.getNumFins");
            myFish = null;
            returnedFish = null;

            // Verify not modifications persist when original object is collected
            createAndStoreFish();
            // Call collect garbage twice. There was an issue on amd64 runs with the object not being collected with one call.
    ...
    Loader42_FileName = "Projected Object Attributes Tests (not configurable)";
})();

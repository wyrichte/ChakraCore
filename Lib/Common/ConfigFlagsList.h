...
PHASE(All)
    ...
    PHASE(Run)
        ...
        PHASE(JSLS)
            PHASE(JSLSScriptTrace)
            PHASE(JsDocParsing)
                PHASE(JsDocParsingErrors)
            PHASE(TaskParsing)
            PHASE(JSLSStats)
            PHASE(DocCommentParseTree)
            PHASE(DirectExecutionParseTree)
            PHASE(CommentTable)
            PHASE(NavBarDynamicAnalysis)
#undef PHASE
#endif
...
#define DEFAULT_CONFIG_ES6Super                (false)
...
#define DEFAULT_CONFIG_FuncObjectWithInlineCacheThreshold   (100)
...
FLAGPR_REGOVR_EXP(Boolean, ES6, ES6Super               , "Enable ES6 'super' keyword"                              , DEFAULT_CONFIG_ES6Super)
...
FLAGPR           (Boolean, ES6, ES6UnicodeVerbose      , "Enable ES6 Unicode 6.0 verbose failure output"           , !BinaryFeatureControl::LanguageService())
...
FLAGNR(Number, FuncObjectWithInlineCacheThreshold, "Maximum number of function objects with inline caches to be created per scriptContext", DEFAULT_CONFIG_FuncObjectWithInlineCacheThreshold)
#ifdef MUTATORS

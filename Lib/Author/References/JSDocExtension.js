(function () {
        ...
        for (var i = 0; i < commentStringArr.length; i++) {
            ...
            switch (tagName) {
                ...
                case "@returns":
                    ...
                    if (this.returnType.indexOf("|") >= 0) this.returnType = this.returnType.replace("|", " | ");
                ...
                case "@param":
                    ...
                    param.name = name[1];
                    if (param.name.indexOf("...") >= 0) handleVarLenthArgParams(param, "name");
                    param.type = typePattern.exec(desArr[1])[1]
                    if (param.type.indexOf("...") >= 0) handleVarLenthArgParams(param, "type");
                    ...
                    if (param.type.indexOf("|") >= 0) param.type = param.type.replace("|", " | ");
    ...
    function handleVarLenthArgParams(param, field) {
        param.isVarLengthArg = true;
        ...
        for (var i = 0; i < doc.params.length; i++) {
            ...
            for (var j = 0; j < sig.params.length; j++) {
                ...
                if (sig.params[j].name == param.name) {
                    ...
                    setPropertiesForParams(param, sig.params[j]);
                    isFound = true;
                    break;
                }
            }
            if (!isFound && sig.params.length == doc.params.length - 1 && param.isVarLengthArg) {
                var tempParam = new Parameter();
                tempParam.name = "...";
                setPropertiesForParams(param, tempParam);
                sig.params.push(tempParam);
            }
        }
        return doc._private || doc._ignore;
    }

    function setPropertiesForParams(docParam, sigParam) {
        sigParam.type = docParam.type;
        sigParam.description = docParam.description;
        sigParam.optional = docParam.isOptional;
    }
})();

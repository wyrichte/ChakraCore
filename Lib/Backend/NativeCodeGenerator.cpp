...
:   JsUtil::WaitableJobManager(scriptContext->GetThreadContext()->GetJobProcessor()),
    ...
    foregroundAllocators(null),
    backgroundAllocators(null),
...
#ifdef PROFILE_EXEC
    , foregroundCodeGenProfiler(null)
    , backgroundCodeGenProfiler(null)
#endif
{
    ...
    if (Js::Configuration::Global.flags.IsEnabled(Js::AsmDumpModeFlag)
        && (Js::Configuration::Global.flags.AsmDumpMode != NULL))
    {
...
{    
...
#ifdef PROFILE_EXEC
    if (this->foregroundCodeGenProfiler != null)
    {
    ...
    {
        Assert(this->backgroundCodeGenProfiler == null);
    }
#endif
...
NativeCodeGenerator::GenerateFunction(Js::FunctionBody *fn, Js::ScriptFunction * function) 
{
    ...
    AutoPtr<JsFunctionCodeGen> workItemAutoPtr(this->NewFunctionCodeGen(fn, null));
    if ((JsFunctionCodeGen*) workItemAutoPtr == null)
    {
        // OOM, just skip this work item and return.
        return;
    }    

    Js::FunctionEntryPointInfo* entryPointInfo = null;
...
void NativeCodeGenerator::GenerateLoopBody(Js::FunctionBody * fn, Js::LoopHeader * loopHeader, Js::EntryPointInfo* entryPoint, uint localCount, Js::Var localSlots[])
{
    ASSERT_THREAD();
    Assert(fn->GetScriptContext()->GetNativeCodeGenerator() == this);        
    Assert(entryPoint->address == NULL);
    ...
    try
    {
        if (!fn->GetIsAsmJsFunction()) // not needed for asmjs as we dont profile in asm mode
        {
...
NativeCodeGenerator::IsValidVar(const Js::Var var, Recycler *const recycler)
{
    ...
    if(typeId != TypeIds_Array)
    {
        ...
        return objectArray == null || recycler->IsValidObject(objectArray, sizeof(*objectArray));
    }
...
NativeCodeGenerator::CodeGen(PageAllocator * pageAllocator, CodeGenWorkItem* workItem, const bool foreground)
{
    ...
    CodeGenAllocators *const allocators =
        foreground ? EnsureForegroundAllocators(pageAllocator) : GetBackgroundAllocator(pageAllocator); // ok to do outside lock since the respective function is called only from one thread

    Js::ScriptContextProfiler *const codeGenProfiler =
#ifdef PROFILE_EXEC
        foreground ? EnsureForegroundCodeGenProfiler() : GetBackgroundCodeGenProfiler(pageAllocator); // ok to do outside lock since the respective function is called only from one thread
#else
        null;
#endif
    ...
    do
    {
        // the number allocator needs to be on the stack so that if we are doing forground JIT
        // the chunk allocated from the recycler will be stacked pinned
        CodeGenNumberAllocator numberAllocator(
            foreground? null : scriptContext->GetThreadContext()->GetCodeGenNumberThreadAllocator(),
...
NativeCodeGenerator::CheckAsmJsCodeGen(Js::ScriptFunction * function)
{
    ...
    // we will need to set the functionbody external and asmjsentrypoint to the fulljit entrypoint
...
NativeCodeGenerator::CheckCodeGen(Js::ScriptFunction * function)
{
    ...
    if(!nativeCodeGen->Processor()->PrioritizeJob(nativeCodeGen, entryPoint, function))
    {
        ...
        // that last episode completes succesfully.
...
NativeCodeGenerator::JobProcessed(JsUtil::Job *const job, const bool succeeded)
{
...
#endif 
            // It's ok if the entry point has been reclaimed at this point 
    ...
    else if (workitemType == JsLoopBodyWorkItemType)
    {
        ...
        {    
            ...
            {
                entryPoint->Reset(!entryPoint->IsJITCapReached()); //reset state to NotScheduled if JIT cap hasn't been reahced
            }           
    ...
    uint32 recursiveInlineDepth)
{
    ...
    Assert(topFunctionBody != null && (!entryPoint->GetWorkItem() || entryPoint->GetWorkItem()->GetFunctionBody() == topFunctionBody));
    Assert(objTypeSpecFldInfoList != null);
...
#define InlineCacheStatsArg(jitTimeData) null
#endif
    ...
    if(profileData)
    {
        ...
        for(uint i = 0; i < functionBody->GetInlineCacheCount(); ++i)
        {
            ...
            if (!isPolymorphic)
            {
                ...
                Js::ObjTypeSpecFldInfo* objTypeSpecFldInfo = null;
            ...
            else
            {
                ...
                if (polymorphicInlineCache != null)
                {
    ...
    for(Js::ProfileId profiledCallSiteId = 0; profiledCallSiteId < profiledCallSiteCount; ++profiledCallSiteId)
    {
        ...
        if(!inlinee)
        {
            ...
            //We should be able to inline atleast two functions here.
        ...
        else 
        {
            ...
            Js::InlineCache * inlineCache = null;
            ...
            if (getSetInlineCandidate ^ callApplyInlineCandidate)
            {
                ...
                Js::FunctionCodeGenRuntimeData *const inlineeRuntimeData = IsInlinee ? runtimeData->EnsureLdFldInlinee(recycler, inlineCacheIndex, inlineeFunctionBody) :
                    ...
                    if (inlineeRuntimeData->GetFunctionBody() != inlineeFunctionBody)
                    {
                        ...
                        //In this case we don't wan't to inline that getter. Polymorphic bit will be set later correctly.
                        //See WinBlue 54540
                        continue;
                    }
    ...
    if (PHASE_VERBOSE_TRACE(Js::ObjTypeSpecPhase, topFunctionBody) || PHASE_VERBOSE_TRACE(Js::EquivObjTypeSpecPhase, topFunctionBody))
    {
        ...
        {
            Output::Print(L"ObTypeSpec: gathered code gen data for function %s (#%u) inlineed %s (#%u): inline cache stats:\n", 
...
{
    Assert(this->foregroundCodeGenProfiler == null);
...
NativeCodeGenerator::SetProfilerFromNativeCodeGen(NativeCodeGenerator * nativeCodeGen)
{
    ...
    Assert(this->foregroundCodeGenProfiler != null);
    Assert(this->foregroundCodeGenProfiler->IsInitialized());
    Assert(nativeCodeGen->foregroundCodeGenProfiler != null);
...
{
    AssertMsg((profiler != null) == Js::Configuration::Global.flags.IsEnabled(Js::ProfileFlag),  
    ...
    if (Js::Configuration::Global.flags.IsEnabled(Js::ProfileFlag))
    {
        ...
        while (codegenProfiler)
        {
            if (codegenProfiler->pageAllocator == allocator)
            {
                ...
                {
                    codegenProfiler->Initialize(allocator, null);
                }

...
LowererMD::LowerTry(IR::Instr *tryInstr, IR::JnHelperMethod helperMethod)
{
    ...
    // Arg 3: frame pointer
    IR::RegOpnd *ebpOpnd = IR::RegOpnd::New(NULL, lowererMDArch.GetRegBlockPointer(), TyMachReg, this->m_func);
...
LowererMD::LowerLeaveNull(IR::Instr *finallyEndInstr)
{
    ...
    IR::RegOpnd *eaxOpnd = IR::RegOpnd::New(NULL, lowererMDArch.GetRegReturn(TyMachReg), TyMachReg, this->m_func);
                              ...
                              IR::Opnd *opndBailOutArg, IR::PropertySymOpnd *propSymOpnd, bool isHelperContinuation)
{
    ...
    if (bailOutInstr != callInstr)
    {
        ...
        {
            this->m_lowerer->GenerateBailOut(bailOutInstr, null, labelBailOut);
        }
        else
        {
...
LowererMD::LowerRet(IR::Instr * retInstr)
{
    ...
    if (m_func->GetJnFunction()->GetIsAsmjsMode() && !m_func->IsLoopBody()) // for loop body ret is the bytecodeoffset
    {
        ...
        retReg = IR::RegOpnd::New(NULL, lowererMDArch.GetRegReturnAsmJs(regType), regType, m_func);
    }
    else
    {
        retReg = IR::RegOpnd::New(NULL, lowererMDArch.GetRegReturn(TyMachReg), TyMachReg, m_func);
    }
    ...
    switch(instr->m_opcode)
    {
        case Js::OpCode::MOV:
        {
            ...
            if(TySize[dstType] > TySize[srcType])
            {
                ...
                IR::IntConstOpnd *const intConstantSrc = src->IsIntConstOpnd() ? src->AsIntConstOpnd() : null;
                ...
                if (!instr->isInlineeEntryInstr)
                {
                    ...
                    if (indirOpnd != nullptr)
                    {
                        ...
                        else
                        {
                            ...
                            // with the reg = MOV <some address wtihin 32-bit range at the start of the function
...
LowererMD::GenerateFastBrString(IR::BranchInstr *branchInstr)
{
    ...
    IR::RegOpnd *srcReg1 = branchInstr->GetSrc1()->IsRegOpnd() ? branchInstr->GetSrc1()->AsRegOpnd() : null;
    IR::RegOpnd *srcReg2 = branchInstr->GetSrc2()->IsRegOpnd() ? branchInstr->GetSrc2()->AsRegOpnd() : null;
    ...
    //      JEQ $helper
    //      MOV s6,[s4]
    //      CMP [s5], s6                       -First charater comparison
    //      JNE $fail
    //      SHL length, 1
    ...
    IR::LabelInstr *labelBranchFail = NULL;
    IR::LabelInstr *labelBranchSuccess = NULL;
    bool isEqual = false;
    bool isStrict = false;
    ...
    //TODO: Change TyUint16 comparision to TyUint32 - requires zero length string to be terminated with two nulls.
...
void LowererMD::GenerateFastCmXx(IR::Instr *instr)
{
    ...
    if (isIntDst)
    {
        // reg = MOV 0 will get peep'd to XOR reg, reg which sets the flags.
...
LowererMD::GenerateFastAdd(IR::Instr * instrAdd)
{
    ...
    {
        // truncate to untag
        int value = reinterpret_cast<int>(opndSrc2->AsAddrOpnd()->m_address);
        if (value == 1)
        {
...
LowererMD::GenerateLoadPolymorphicInlineCacheSlot(IR::Instr * instrLdSt, IR::RegOpnd * opndInlineCache, IR::RegOpnd * opndType, uint polymorphicInlineCacheSize)
{
    ...
    instrLdSt->InsertBefore(instr);
    instr = IR::Instr::New(Js::OpCode::AND, opndOffset, opndOffset, IR::AddrOpnd::New((void*)((polymorphicInlineCacheSize - 1) << leftShiftAmount), IR::AddrOpndKindConstant, instrLdSt->m_func, true), instrLdSt->m_func);
    instrLdSt->InsertBefore(instr);
...
LowererMD::GenerateFastAbs(IR::Opnd *dst, IR::Opnd *src, IR::Instr *callInstr, IR::Instr *insertInstr, IR::LabelInstr *labelHelper, IR::LabelInstr *doneLabel)
{
    ...
    if (emitFloatAbs)
    {
        ...
        // Unconditionally set the sign bit. This'd get XORd away when we remove the tag.
...
//
void LowererMD::GenerateSmIntTest(IR::Opnd *opndSrc, IR::Instr *insertInstr, IR::LabelInstr *labelHelper, IR::Instr **instrFirst /* = null */, bool fContinueLabel /*= false*/)
{
...
void LowererMD::HelperCallForAsmMathBuiltin(IR::Instr* instr, IR::JnHelperMethod helperMethodFloat, IR::JnHelperMethod helperMethodDouble)
{
    ...
    case Js::OpCode::InlineMathCeil:
    {
        ...
        if (isFloat32)
        {
            ...
            dst1 = IR::RegOpnd::New(null, (RegNum)FIRST_FLOAT_ARG_REG, TyFloat32, this->m_func);
        }
        else
        {
            argOut = IR::Instr::New(Js::OpCode::MOVSD, this->m_func);
            dst1 = IR::RegOpnd::New(null, (RegNum)FIRST_FLOAT_ARG_REG, TyMachDouble, this->m_func);
        }
    ...
    case Js::OpCode::InlineMathTan:
        {
            ...
            // Call CRT.
            IR::RegOpnd* floatCallDst = IR::RegOpnd::New(null, (RegNum)(FIRST_FLOAT_REG), TyMachDouble, this->m_func);   // Dst in XMM0.
#ifdef _M_IX86
...
#else
    IR::RegOpnd* regXMM0 = IR::RegOpnd::New(null, (RegNum)FIRST_FLOAT_ARG_REG, TyMachDouble, this->m_func);
...
void LowererMD::GenerateFastInlineBuiltInMathAbs(IR::Instr* inlineInstr)
{
    ...
    else if (srcType == IRType::TyFloat64)
    {
        if (!dst->IsRegOpnd())
        {
            ...
            IR::RegOpnd* tempRegOpnd = IR::RegOpnd::New(null, TyMachDouble, this->m_func);
    ...
    else if (srcType == IRType::TyFloat32)
    {
        if (!dst->IsRegOpnd())
        {
            ...
            IR::RegOpnd* tempRegOpnd = IR::RegOpnd::New(null, TyFloat32, this->m_func);
            tempRegOpnd->m_isCallArg = true; // This is to make sure that lifetime of opnd is virtually extended until next CALL instr.

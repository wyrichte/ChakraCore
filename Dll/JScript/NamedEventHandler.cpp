...
HRESULT ScriptEngine::RegisterEventHandler(__in IDispatch* pDispatch, __in  LPWSTR eventName, __in  Js::DynamicObject* scriptObject)
{
    HRESULT hr = S_OK;
    
    // Handle registering an event against an object in a different context
    BEGIN_TRANSLATE_OOM_TO_HRESULT
    {
        scriptObject = Js::DynamicObject::FromVar(Js::CrossSite::MarshalVar(GetScriptContext(), scriptObject));
    }
    END_TRANSLATE_OOM_TO_HRESULT(hr);
    ...
    IBindEventHandler* bindEvent;
    if (SUCCEEDED(hr = pDispatch->QueryInterface(IID_IBindEventHandler, (void**)&bindEvent)))
    {
        BEGIN_TRANSLATE_OOM_TO_HRESULT
        {
            JavascriptDispatch* javascriptDispatch = JavascriptDispatch::Create<false>(scriptObject);
            hr = bindEvent->BindHandler(eventName, javascriptDispatch);
        }
        END_TRANSLATE_OOM_TO_HRESULT(hr);
        bindEvent->Release();
        return hr;
    ...
    if (FAILED(hr = ObjectEventHandler::Create(&objectEventHandler, this, pDispatch, eventName, scriptObject)))
    {
        return hr;
    }
    ...
    BEGIN_TRANSLATE_OOM_TO_HRESULT
    {
        eventHandlers->Add(objectEventHandler);
    }
    END_TRANSLATE_OOM_TO_HRESULT(hr);
    if (FAILED(hr))
    {
        objectEventHandler->Release();
        return hr;
    }
...
***************************************************************************/
ObjectEventHandler::ObjectEventHandler(void)
{
    refCount = 1;
    scriptEngine = NULL;    
    pdisp= NULL;
...
ObjectEventHandler::~ObjectEventHandler(void)
{
    ...
    if (scriptObject)
    {
        scriptObject.Unroot(scriptObject->GetScriptContext()->GetRecycler());        
    }
}


HRESULT ObjectEventHandler::Create(
   __out ObjectEventHandler **ppoeh, 
   __in ScriptEngine *pos,
   ...
   __in Js::DynamicObject* scriptObject)
{
    ...
    if (NULL == (*ppoeh = new ObjectEventHandler))
        return HR(E_OUTOFMEMORY);
    if (FAILED(hr = (*ppoeh)->Init(pos, pdisp, eventName, scriptObject)))
    {
        (*ppoeh)->Release();
        *ppoeh = NULL;
        return hr;
...
Js::DynamicObject* ObjectEventHandler::GetScriptObject()
{
    return scriptObject;
}

HRESULT ObjectEventHandler::Init(__in ScriptEngine *pos, __in IDispatch *pdisp, __in LPWSTR eventName,
                            __in Js::DynamicObject *scriptObject)
{
    scriptEngine = pos;
    this->pdisp= pdisp;
    ...
    BEGIN_TRANSLATE_OOM_TO_HRESULT
    {        
        this->scriptObject.Root(scriptObject, scriptEngine->GetScriptContext()->GetRecycler());        
    }
    END_TRANSLATE_OOM_TO_HRESULT(hr);
...
HRESULT ObjectEventHandler::Reset(void)
{
    // Shouldn't happen.
    AssertMsg(FALSE, "Who is telling us to Reset?");
    return NOERROR;
}


HRESULT ObjectEventHandler::Clone(__in ScriptEngine *pos, __deref_out BaseEventHandler **ppeh) 
{
    // Shouldn't happen.
    AssertMsg(FALSE, "Who is telling us to Clone?");

...
// Object Event Handler. Comes from an event handler declared in code.
class ObjectEventHandler sealed : public BaseEventHandler
{
private:
    long refCount;
    ...
    ObjectEventHandler(void);
    ~ObjectEventHandler(void);

public:
    static HRESULT Create(__in ObjectEventHandler **ppoeh, 
        __in ScriptEngine *scriptEngine,
        __in IDispatch *pdisp, 
        __in LPWSTR eventName, 
        __in Js::DynamicObject *scriptObject);

    HRESULT Init(__in ScriptEngine* scriptEngine, __in IDispatch* dispatch, __in LPWSTR eventName,
                 __in Js::DynamicObject* scriptObject);
    ...
    virtual HRESULT Clone(__in ScriptEngine *pos, __deref_out BaseEventHandler **ppeh) override;
    LPCOLESTR GetEventHandlerName() { return eventName; }
    BOOL ShouldPersist(){ return FALSE; }
    IDispatch* GetDispatch() { return pdisp; }
};

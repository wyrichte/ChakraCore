:: ============================================================================
::
:: runwinrt2tstests.cmd
::
:: Runs WinRt2Ts unittests.
::
:: ============================================================================
...
:printUsage
  ...
  echo runwinrt2tstests.cmd -x86^|-x64 -debug^|-test
  echo.
  echo Required switches:
  echo.
  echo   Specify architecture of winrt2ts binaries (required):
  echo.
  echo   -x86           Build arch of binaries is x86
  echo   -x64           Build arch of binaries is x64
  echo.
  echo   Specify type of winrt2ts binaries (required):
  echo.
...
:printGetHelp
  ...
  echo For help use runwinrt2tstests.cmd -?
...
:parseArgs
  ...
  if not "%1" == "" echo -- runwinrt2tstests.cmd ^>^> Unknown argument: %1 & set fShowGetHelp=1
...
:validateArgs
  ...
  if "%_BuildArch%" == "" (
    echo -- runwinrt2tstests.cmd ^>^> Error missing required build architecture or build type switch
    set fShowGetHelp=1
    goto :eof
  )
...
:configureVars
  ...
  rem Use %_BinDir% as the root for %_TestTempDir% to ensure that running
  ...
  set _TestTempDir=%_BinDir%winrt2tstests\
  if not exist %_TestTempDir% mkdir %_TestTempDir%

  echo -- runwinrt2tstests.cmd ^>^> #### BinDir: %_BinDir%
  echo -- runwinrt2tstests.cmd ^>^> #### ObjDir: %_ObjDir%
  echo -- runwinrt2tstests.cmd ^>^> #### TestTempDir: %_TestTempDir%

  goto :eof

:: ============================================================================
:: Copying winmd files from source location to temp test location and test
:: binaries from binary location to test temp location.
:: ============================================================================
:copyWinMdsAndBinaries

  echo -- runwinrt2tstests.cmd ^>^> copying winmds to '%_TestTempDir%'
...
:runtest
  ...
  set _TestBinary=%_AllTestBins% %_TestTempDir%WinRt2TsTest.dll
  ...
  call :do %_TECommand% %_TestBinary%  /select:"not(@Disabled='Yes')" /unicodeoutput:false /parallel:1 /logOutput:low /p:TestMetadataWinmd=%_TestTempDir%TestMetadata.winmd /p:WindowsFoundationWinmd=%_TestTempDir%windows.foundation.winmd

	...
	if (-not (Test-Path($global:gumshoeExe)))
  	{
    		...
    		if (-not (Test-Path($global:gumshoeExe)))
    		{
      			Write-Host "Failed to install Gumshoe :("  -ForegroundColor Red-ForegroundColor Red
      			return $false
    		}
  	}
  	else
...
Function Setup-CodeCoverage
{
	...
	{
		Write-Host "Couldn't find the binary at " $binary -Foregroundcolor Red
		return $false
	}
	...
	Write-Host "Renaming the instrumented binary $binInstrName to $binary"
  	...
  	# TODO: Take care of the chakrals case??

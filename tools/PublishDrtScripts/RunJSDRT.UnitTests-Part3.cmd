...
echo powershell.exe -ExecutionPolicy Bypass %BUNDLEROOT%\RunJSDRT.ps1 -RunUnitTests $True -RunHtmlUnitTests $False -RunJsrtUnitTests $False -DrtBundleRoot %BUNDLEROOT% -DrtMode $True -BuildArch %ARCH% -BuildFlavor %FLAVOR% -LogsRootDir %LOGSDIR% -Dirs es6`,Profiler`,Strings`,Operators`,loop`,bailout`,sca`,Basics`,Lib`,Miscellaneous`,Regex`,UnifiedRegex`,utf8
powershell.exe -ExecutionPolicy Bypass %BUNDLEROOT%\RunJSDRT.ps1 -RunUnitTests $True -RunHtmlUnitTests $False -RunJsrtUnitTests $False -DrtBundleRoot %BUNDLEROOT% -DrtMode $True -BuildArch %ARCH% -BuildFlavor %FLAVOR% -LogsRootDir %LOGSDIR% -Dirs es6`,Profiler`,Strings`,Operators`,loop`,bailout`,sca`,Basics`,Lib`,Miscellaneous`,Regex`,UnifiedRegex`,utf8

...
echo powershell.exe -ExecutionPolicy Bypass %BUNDLEROOT%\RunJSDRT.ps1 -RunProjectionTests $True -RunUnitTests $False -RunHtmlUnitTests $False -RunJsrtUnitTests $False -DrtBundleRoot %BUNDLEROOT% -DrtMode $True -BuildArch %ARCH% -BuildFlavor %FLAVOR% -LogsRootDir %LOGSDIR% -Dirs Configurable`,HeapDump`,Generated`,ScriptErrorTests`,MemoryTracing`,Versioning`,MetaDataParsing
powershell.exe -ExecutionPolicy Bypass %BUNDLEROOT%\RunJSDRT.ps1 -RunProjectionTests $True -RunUnitTests $False -RunHtmlUnitTests $False -RunJsrtUnitTests $False -DrtBundleRoot %BUNDLEROOT% -DrtMode $True -BuildArch %ARCH% -BuildFlavor %FLAVOR% -LogsRootDir %LOGSDIR% -Dirs Configurable`,HeapDump`,Generated`,ScriptErrorTests`,MemoryTracing`,Versioning`,MetaDataParsing

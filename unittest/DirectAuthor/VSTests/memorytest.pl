...
# test class
my @variants = ("MemoryTests", "ArenaMemoryTests");

# test name -- each line maps to a variant
my @tests = (
              # MemoryTests Tests
              ["Memory_ContextFiles", "Memory_GetGlobalCompletions", "Memory_GetMessages", "Memory_GetParameterHelp", "Memory_GetRegions", "Memory_GetSimpleCompletions", "Memory_JQueryTest"],
              # ArenaMemory Tests
              ["SimulateArenaMemoryForWWATest"]
             );
...
for (my $variantIndex = 0; $variantIndex < scalar(@tests); $variantIndex++)
{
    ...
    if(!$is_baseline)
    {
        printf("TEST - %-10s        BASE(bytes)         TEST(bytes)     DIFF(bytes)  RATIO(%)\n", uc($variant), 0);
        print("---------------------------------------------------------------------------\n");
    }
    else
    {
    ...
    for (my $variantIndex = 0; $variantIndex < scalar(@tests); $variantIndex++)
    {
      ...
      foreach my $test(@testlist)
      {
          for(my $i = 0; $i < $iter; ++$i)
          {
              ...
              my $SQL= "insert into MemoryTestData (TimeStamp, Changeset, ClassName, TestCaseName, Iteration, Memory)" .

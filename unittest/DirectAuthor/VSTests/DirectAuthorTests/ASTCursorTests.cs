...
namespace DirectAuthorTests
{
    ...
    public class ASTCursorTests : DirectAuthorTest
    {
        ...
        {
            IAuthorParseNodeSet serializedNodes = cursor.GetSubTree();
        ...
        [TestMethod]
        public void GetRegionsComparison()
        {
            var file = _session.ReadFile(Paths.JQuery.JQuery_1_2_6VSDocFilePath);
            var context = _session.OpenContext(file);
            context.Update();

            // native
            var expectedRegions = context.GetRegions();

            // managed
            var cursor = context.GetASTCursor();
            RangeWalker walker = new RangeWalker(cursor);
            walker.Walk();

            Assert.IsTrue(expectedRegions.Count == walker.Regions.Count);
            AssertAreStructurallyEqual(expectedRegions, walker.Regions);
        }
...
",
    ...
    internal class RangeWalker : ParseTreeWalker
    {
        private List<AuthorFileRegion> regions;

        public RangeWalker(IAuthorParseNodeCursor cursor)
            : base(cursor)
        {
            regions = new List<AuthorFileRegion>();
        }

        public override void VisitNode(ref AuthorParseNodeDetails node, AuthorParseNodeEdge childLabel)
        {
            if (node.Kind == AuthorParseNodeKind.apnkFncDecl)
            {
                int ichRParenMin = cursor.GetNodeProperty(AuthorParseNodeProperty.apnpRParenMin) + 1;
                AuthorFileRegion region = new AuthorFileRegion();
                region.Offset = ichRParenMin;
                region.Length = node.EndOffset - ichRParenMin;
                regions.Add(region);
            }
        }

        public List<AuthorFileRegion> Regions
        {

...
namespace DirectAuthorTests
{
    ...
    {
        protected static readonly string DeferredParsingFilePath = Path.Combine(Paths.FilesPath, "DeferredParsing.js");
...
(function(global) {
   ...
   global.init = init;
        ...
        new internal void PerformCompletionRequests(string text, Action<IEnumerable<AuthorCompletion>, string, int> action, AuthorCompletionFlags flags = AuthorCompletionFlags.acfMembersFilter, params string[] contextFiles)
        {
            ...
            {
                var forceDeferredParsing = _session.ReadFile(DeferredParsingFilePath).Text;

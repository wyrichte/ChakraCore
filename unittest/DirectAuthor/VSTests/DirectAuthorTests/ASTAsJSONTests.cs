...
var b = [""foo"", ""bar""];";
   ...
   ""body"":[
        ...
        [TestMethod]
        [WorkItem(508883)]
        public void VerifySerializationOfLetAndConst()
        {
            var file = _session.FileFromText(VerifySerializationOfLetAndConstText);
            var context = _session.OpenContext(file);

            var actual = context.GetASTAsJSON();
            VerifyJSON(actual, VerifySerializationOfLetAndConst_Expected);
        }
        #region Test data
        const string VerifySerializationOfLetAndConstText = @"
function foo() {
  const a = 1;
  let b = 2;
  ...
  {  
    ...
    if (c) doSomething(d, e, f, g, h);
  }
}
";
        const string VerifySerializationOfLetAndConst_Expected = @"
{
  ""__type"": ""Unit"", 
  ""offset"": 0, 
  ""length"": 200, 

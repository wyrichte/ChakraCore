...
setTimeout(ready);";
    ...
    public class CompletionTests : CompletionsBase
    {
        ...
        public void JQueryFormElements()
        {
            ...
            var dom = _session.ReadFile(Paths.DomWebPath).Text;
            var jquery = _session.ReadFile(Paths.JQuery.JQuery_1_7_0FilePath).Text;
            PerformRequests(@"
                intellisense.addEventListener('statementcompletion',  function() { });
                var form = document.createElement('form');
                form.name = 'form1';
        ...
        {
            var dom = _session.ReadFile(Paths.DomWebPath).Text;
            var jquery = _session.ReadFile(Paths.JQuery.JQuery_1_7_2FilePath).Text;
        ...
        public void JQueryElementIdSelector()
        {
            var siteTypes = _session.ReadFile(Paths.SiteTypesWindowsPath).Text;
            var dom = _session.ReadFile(Paths.DomWindowsPath).Text;
            var jquery = _session.ReadFile(Paths.JQuery.Latest).Text;
            var generatedElemnts = @"
                (function(window){
                    // ----- GENERATED ELEMENTS FROM REFERENCED HTML DOCUMENTS ------
                    window.document._$documentElements = {
        ...
        public void ArrayTypeAttribute()
        {
            ...
            PerformCompletionRequests(@"
                intellisense.addEventListener('statementcompletion',  function() { });
                function testFoo(i8,i16,i32,ui8,ui16,ui32,f32,f64,nontyped) { 
        ...
        {
            var domjs = _session.ReadFile(Paths.DomWebPath).Text;
            var jQuery = _session.ReadFile(Paths.JQuery.JQuery_1_5_1FilePath).Text;
            PerformRequests("$.| ; $().|", (context, offset, data, index) =>
            {
                var completions = context.GetCompletionsAt(offset);
        ...
        {
            var domjs = _session.ReadFile(Paths.DomWebPath).Text;
            var jQuery = _session.ReadFile(Paths.JQuery.JQuery_1_6_1VSDocFilePath).Text;
        ...
        {
            var domjs = _session.ReadFile(Paths.DomWebPath).Text;
            var mootools = _session.ReadFile(Path.Combine(Paths.FilesPath, @"mootools-core-1.3.2-full-compat.js")).Text;
...
jQuery.each([ ""Height"", ""Width"" ], function( i, name ) {
	...
	jQuery.fn[ type ] = function( size ) {
        ...
        {
            var primary = _session.ReadFile(Path.Combine(Paths.FilesPath, @"Bug245383.js"));
...
test();";
        ...
        public void PrototypeJsExtendCompletion()
        {
            ...
            ", (context, offset, data, index) =>
             ...
             }, false, Paths.DomWebPath, Path.Combine(Paths.FilesPath, @"prototype.js"));
        }
...
break nhb;
        ...
        {
            var primary = _session.ReadFile(Path.Combine(Paths.FilesPath, @"Bug245387.js"));
            var context = _session.OpenContext(primary);
            var completions = context.GetCompletionsAt(primary.Text.Length);
            Assert.IsNotNull(completions);
        }
        ...
        public void IntlCompletions()
        {
            PerformCompletionRequests(IntlCompletionsTest, (completions, data, i) =>
            {
                switch (data)
                {
                    ...
                    case "Intl": completions.ExpectContains(IntlMethods); break;
...
";
        ...
        [TestMethod]
        [WorkItem(508883)]
        public void VerifyLetConstInCompletionLists()
        {
            // TODO: Update test when the "let i" below is in the correct scope (that is when, the for gets it own scope)
            PerformCompletionRequests(VerifyLetConstInCompletionListsText);
        }
        #region Test data
        const string VerifyLetConstInCompletionListsText = @"
let gl1 = 1;
const gc1 = 1;

function foo(p1, p2) {
  let fool1 = 1, fool2 = 1;
  const fooc1 = 1, fooc2 = 1;

  {
    let foobl1 = 1;
    const foobc1 = 1;

|gl1,gc1,p1,p2,fool1,fool2,fooc1,fooc2,foobl1,foobc1|
  }
|gl1,gc1,p1,p2,fool1,fool2,fooc1,fooc2,!foobl1,!foobc1|
  ...
  switch (p1) {
    case 1:
      let foosl1 = 1;
      const foosc1 = 1;
|gl1,gc1,p1,p2,fool1,fool2,fooc1,fooc2,foosl1,foosc1|
      break;
  }
|gl1,gc1,p1,p2,fool1,fool2,fooc1,fooc2,!foosl1,!foosc1|
  ...
  for (let i = 0; i < 10; i++) {
    let foofl1 = 1;
    const foofc1 = 1;
|gl1,gc1,p1,p2,fool1,fool2,fooc1,fooc2,i,foofl1,foofc1|
  }
|gl1,gc1,p1,p2,fool1,fool2,fooc1,fooc2,!foofl1,!foofc1|
}
...
";
    ...
    public class DomTraversalTests : CompletionsBase
    {
        ...
        {
            var dom = _session.ReadFile(Paths.DomWebPath).Text;
            var jquery = _session.ReadFile(Path.Combine(Paths.FilesPath, @"Bug271685\07.jQueryv1.3.2.js")).Text;
        ...
        {
            PerformCompletionRequests("|NaN|", "!!" + Path.Combine(Paths.FilesPath, @"515673\PP.js"));
        }

    ...
    public class QuickInfoTests : DirectAuthorTest
    {
        ...
        public void FieldDocCommentInCtor()
        {
            ...
            (context, offset, data, index) =>
            {
                ...
                var compatibleWithSet = help.GetCompatibleWith();
                Assert.IsNotNull(compatibleWithSet);
                var compatibleWith = compatibleWithSet.ToEnumerable();
                compatibleWith.Count().Expect(1);
                var compatItem = compatibleWith.First();
                Assert.IsNotNull(compatItem);
                compatItem.Platform.Expect("Windows");
                compatItem.MinVersion.Expect("8.1");
            });
        ...
        public void FunctionDocComments()
        {
            PerformRequests(@"
                ...
                NS.Class1 = function() {
                    ...
                    /// <compatibleWith platform='WindowsPhoneAppx' minVersion='8.1.2.0' />
                    /// </signature>
                    function make() { }
                    return new make();
                };
            ...
            (context, offset, data, index) =>
            {
                ...
                var compatibleWithSet = signature.GetCompatibleWith();
                Assert.IsNotNull(compatibleWithSet);
                var compatibleWith = compatibleWithSet.ToEnumerable();
                compatibleWith.Count().Expect(1);
                var compatItem = compatibleWith.First();
                Assert.IsNotNull(compatItem);
                compatItem.Platform.Expect("WindowsPhoneAppx");
                compatItem.MinVersion.Expect("8.1.2.0");
            });
        }

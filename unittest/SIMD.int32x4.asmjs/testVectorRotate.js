function asmModule(stdlib, imports, buffer) {
	...
	function rotate(start, n, k) {
		...
		var SIMD_SIZE = 4;
		var c = 0, tmp = i4(0, 0, 0, 0), v = 0, t = 0, tp = 0;
		
		for(v = 0; c < n; v = v + 1) {
			t = v|0;
			tp = (v + k)|0;
			tmp = i4load(Int32Heap, start + v << 2 >> 2);
			c = c + SIMD_SIZE;
			while ( tp != v) {
				i4store(Int32Heap, start + t << 2 >> 2, i4load(Int32Heap, start + tp << 2 >> 2));
				t = tp|0;
				tp = (tp + k)|0;
				if(tp >= n) {
					tp = (tp - n)|0;
			...
			i4store(Int32Heap, start + t << 2 >> 2, tmp);
		}
	}
...
var m = asmModule(this, {g0:initI32(buffer),g1:SIMD.Float32x4(9,9,9,9), g2:SIMD.Int32x4(1, 2, 3, 4), g3:SIMD.Float64x2(10, 10, 10, 10)}, buffer);

...
var entries = ["-", "a", "c", "\\s", "a-c", "c-a", "\\s-a", "a-\\s", "\\s-c", "c-\\s", "\\s-\\w", "a-", "c-", "\\s-", "-a", "-c", "-\\s"]
...
function build(path) {
    ...
    for (var i = 0; i < path.length; i++)
        sb.push(entries[path[i]]);
    return sb.join("");
}

for (var numEnts = 0; numEnts < 5; numEnts++) {
    echo("---- " + numEnts + " ----");
    var path = zero(numEnts);
    do {
        var str = "[" + build(path) + "]";
        try {
            new RegExp(str, "");

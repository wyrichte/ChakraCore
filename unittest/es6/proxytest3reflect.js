...
var tests = [
    ...
    },
    {
        ...
        body: function () {
            ...
            if (handler == observeHandler) {
                ...
                assert.areEqual(savedLogResult.length, 3, "3 trap on set when using Reflect");
            }
            assert.areEqual(savedLogResult[0], "set trap 1");
            assert.areEqual(savedLogResult[1], "getOwnPropertyDescriptor trap 1");
            assert.areEqual(savedLogResult[2], "defineProperty trap 1");
        }
    },
    {
        name: "set element i #2",
        body: function () {
            cleanupBeforeTestStarts();
            target['test5'] = 22;
    ...
    },
    {
        ...
        body: function () {
            ...
            assert.throws(function () { observerProxy.mySetter3 = 43; }, TypeError);
            assert.areEqual(savedLogResult.length, 1, "one trp on set");

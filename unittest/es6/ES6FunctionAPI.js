...
var tests = [
    ...
    {
        name: "Returned value from toMethod() linked to an object/classe as a method should be able to access 'this' and 'super' references",
        body: function() {
            class BASE1 {
                base() { return "BASE1"; }
                static base() { return "SBASE1"; }
            }
            class BASE2 {
                base()  {return "BASE2"; }
                static base() { return "SBASE2"; }
            }
            ...
            function testFunctionToMethod(x, s)
            {
                let ret;
                let a=new A();
                let b=new B();
                
                A.prototype.f=x.toMethod(A.prototype);
                B.prototype.f=x.toMethod(B.prototype);
                A.sf=x.toMethod(A);
                B.sf=x.toMethod(B);
                
                assert.throws(function () { x(); }, ReferenceError,
                    s+" called as is should throw ReferenceError", "'super' can only be accessed from a subclass method");
                assert.areEqual(["BASE1","A"], a.f(), s+" linked as a dynamic method of object 'a' through toMethod()");
                assert.areEqual(["BASE2","B"], b.f(), s+" linked as a dynamic method of object 'b' through toMethod()");
                assert.areEqual(["SBASE1","SA"], A.sf(), s+" linked as a static method of class 'A' through toMethod()");
                assert.areEqual(["SBASE2","SB"], B.sf(), s+" linked as a static method of class 'B' through toMethod()");
            }

           function testGeneratorFunctionToMethod(x, s)
            {
                let ret;
                let a=new A();
                let b=new B();
                ...
                assert.throws(function () { x().next().value; }, ReferenceError,
                    s+" called as is should throw ReferenceError", "'super' can only be accessed from a subclass method");
                assert.areEqual(["BASE1","A"], a.f().next().value, s+" linked as a dynamic method of object 'a' through toMethod()");
                assert.areEqual(["BASE2","B"], b.f().next().value, s+" linked as a dynamic method of object 'b' through toMethod()");
                assert.areEqual(["SBASE1","SA"], A.sf().next().value, s+" linked as a static method of class 'A' through toMethod()");
                assert.areEqual(["SBASE2","SB"], B.sf().next().value, s+" linked as a static method of class 'B' through toMethod()");
            }
            
            testFunctionToMethod( function () { return [super.base(),this.id()]; },
                "a function with 'this' and 'super' reference" );
            testFunctionToMethod( function () { x=()=>[super.base(),this.id()]; return x(); },
                "a function with 'this' and 'function' reference in a lambda"  );
            testFunctionToMethod( new Function("return [super.base(),this.id()];"),
                "a function created by new Function()");
             
            testGeneratorFunctionToMethod( function* () { yield [super.base(),this.id()]; },
                "a generator function with 'this' and 'super' reference" );
            testGeneratorFunctionToMethod( function* () {  x=()=>[super.base(),this.id()]; yield x();},
                "a generator function with 'this' and 'function' reference in a lambda" );
        }
    },
    {

...
var tests = [
...

    {
        ...
        body: function() {
            ...
            var copymap = WScript.CopyOnWrite(map);
            var copyfour = WScript.CopyOnWrite(four);
            var copyo = WScript.CopyOnWrite(o);

            assert.isTrue(map.has(1), "original map is unmodified, still has 1");
            assert.isTrue(map.has(2), "original map is unmodified, still has 2");
            assert.isTrue(map.has("three"), "original map is unmodified, still has \"three\"");
            assert.isTrue(map.has(four), "original map is unmodified, still has 'four' object");
            assert.isTrue(map.has(true), "original map is unmodified, still has 'true'");
            assert.isTrue(map.has(undefined), "original map is unmodified, still has 'undefined'");
            assert.isTrue(map.get(10) === 20, "original map is unmodified, still has 10 mapping to 20");
            assert.isTrue(map.get(20) === o, "original map is unmodified, still has 20 mapping to 'o' object");

            assert.isTrue(copymap.has(1), "copy map has 1");
            assert.isTrue(copymap.has(2), "copy map has 2");
            assert.isTrue(copymap.has("three"), "copy map has \"three\"");
            assert.isFalse(copymap.has(four), "copy map does not have 'four' object");
            assert.isTrue(copymap.has(copyfour), "copy map has copyfour object");
            assert.isTrue(copymap.has(true), "copy map has 'true'");
            assert.isTrue(copymap.has(undefined), "copy map has 'undefined'");
            assert.isTrue(copymap.get(10) === 20, "copy map has 10 and it maps to 20");
            assert.isFalse(copymap.get(20) === o, "copy map has 20 but it does not map to 'o' object");
            assert.isTrue(copymap.get(20) === copyo, "copy map has 20 and it maps to 'copyo' object");
            ...
            copymap.set(6);
            copymap.set(null);
            copymap.delete(undefined);
            copymap.set(five);
            copymap.delete(10);
            copymap.set(20, p);

            assert.isTrue(map.has(1), "original map is still unmodified, still has 1");
            assert.isTrue(map.has(2), "original map is still unmodified, still has 2");
            assert.isTrue(map.has("three"), "original map is still unmodified, still has \"three\"");
            assert.isTrue(map.has(four), "original map is still unmodified, still has 'four' object");
            assert.isTrue(map.has(true), "original map is still unmodified, still has 'true'");
            assert.isTrue(map.has(undefined), "original map is still unmodified, still has 'undefined'");
            assert.isTrue(map.get(10) === 20, "original map is still unmodified, still has 10 mapping to 20");
            assert.isTrue(map.get(20) === o, "original map is still unmodified, still has 20 mapping to 'o' object");
            assert.isFalse(map.has(6), "original map is still unmodified, does not have 6");
            assert.isFalse(map.has(null), "original map is still unmodified, does not have 'null'");
            assert.isFalse(map.has(five), "original map is still unmodified, does not have 'five' object");

            assert.isTrue(copymap.has(1), "copy map still has 1");
            assert.isFalse(copymap.has(2), "copy map no longer has 2");
            assert.isTrue(copymap.has("three"), "copy map still has \"three\"");
            assert.isFalse(copymap.has(four), "copy map still does not have four object");
            assert.isTrue(copymap.has(copyfour), "copy map still has copyfour object");
            assert.isTrue(copymap.has(true), "copy map still has 'true'");
            assert.isFalse(copymap.has(undefined), "copy map no longer has 'undefined'");
            assert.isTrue(copymap.has(6), "copy map also now has 6");
            assert.isTrue(copymap.has(null), "copy map also now has 'null'");
            assert.isTrue(copymap.has(five), "copy map also now has 'five' object");
            assert.isFalse(copymap.has(10), "copy map no longer has 10");
            assert.isTrue(copymap.get(10) === undefined, "copy map no longer has 10; get returns undefined");
            assert.isTrue(copymap.get(20) === p, "copy map still has 20 and now it maps to p");
        }
    },
